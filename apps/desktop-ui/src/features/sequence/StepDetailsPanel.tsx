    ...
    <CollapsibleSection title="Action Settings" className="flex min-h-0 flex-1 flex-col overflow-hidden" contentClassName="min-h-0 flex-1 overflow-hidden">
      <div className="flex max-h-full min-h-0 flex-col gap-4">
        <div className="action-settings-scroll grid min-h-0 flex-1 gap-4 overflow-y-auto px-1 pr-3">
          ...
          {step.type === "pixel_check" ? <><CaptureActionRow title="Pixel Sampler" info="Arms pixel sampling; next click captures current color at that location." waiting={samplingPixel} idleLabel="Sample Pixel" pendingHint="Esc cancels" onCapture={onSamplePixel} /><FieldRow title="Mismatch Mode" info="Action to take when color does not match."><Select value={step.mode} onChange={(event) => onChange({ mode: event.target.value as "wait_until_match" | "wait_until_mismatch" | "stop_if_mismatch" | "skip_if_mismatch" | "exit_loop_when_match" })}><option value="wait_until_match">wait until match</option><option value="wait_until_mismatch">wait until mismatch</option><option value="stop_if_mismatch">stop if mismatch</option><option value="skip_if_mismatch">skip if mismatch</option><option value="exit_loop_when_match">exit loop when match</option></Select></FieldRow><FieldRow title="Pixel X" info="Horizontal screen coordinate inspected by this pixel check."><Input aria-label="Pixel X" type="number" value={step.x} onChange={(event) => onChange({ x: Number(event.target.value) })} /></FieldRow><FieldRow title="Pixel Y" info="Vertical screen coordinate inspected by this pixel check."><Input aria-label="Pixel Y" type="number" value={step.y} onChange={(event) => onChange({ y: Number(event.target.value) })} /></FieldRow></> : null}
          ...
          {step.type === "move" ? <><FieldRow title="Mouse X" info="Horizontal screen coordinate."><Input type="number" value={step.x} onChange={(event) => onChange({ x: Number(event.target.value) })} /></FieldRow><FieldRow title="Mouse Y" info="Vertical screen coordinate."><Input type="number" value={step.y} onChange={(event) => onChange({ y: Number(event.target.value) })} /></FieldRow><FieldRow title="Move Transition" info="Instant jumps directly. Smooth is arc-based; Natural is imperfect straight-line movement with optional overshoot correction."><Select aria-label="Move Transition" value={step.movement_mode} onChange={(event) => onChange({ movement_mode: event.target.value as "instant" | "smooth" | "natural" })}><option value="instant">Instant</option><option value="smooth">Smooth</option><option value="natural">Natural</option></Select></FieldRow>{step.movement_mode === "smooth" ? <><FieldRow title="Move Duration (ms)" info="Smooth movement duration in milliseconds. Use 0 to calculate a human-like duration from travel distance."><Input aria-label="Move Duration" type="number" min={0} max={10000} value={step.movement_duration_ms} onChange={(event) => onChange({ movement_duration_ms: Number(event.target.value) })} /></FieldRow><FieldRow title="Smoothness" info="Higher values increase easing and arc strength."><Input aria-label="Move Smoothness" type="range" min={0} max={100} value={step.movement_smoothness} onChange={(event) => onChange({ movement_smoothness: Number(event.target.value) })} /></FieldRow><FieldRow title="Path Randomness" info="Randomizes arc shape and timing without changing the final target coordinate."><Input aria-label="Move Path Randomness" type="range" min={0} max={100} value={step.path_randomness} onChange={(event) => onChange({ path_randomness: Number(event.target.value) })} /></FieldRow><FieldRow title="Arc Direction" info="Direction of the smooth movement arc relative to the travel line."><Select aria-label="Move Arc Direction" value={step.arc_direction} onChange={(event) => onChange({ arc_direction: event.target.value as "auto" | "left" | "right" })}><option value="auto">auto</option><option value="left">left</option><option value="right">right</option></Select></FieldRow><CoordinatePreview x={step.x} y={step.y} offset={0} exactTarget /></> : step.movement_mode === "natural" ? <><FieldRow title="Move Duration (ms)" info="Natural movement duration in milliseconds. Use 0 to auto-size by distance."><Input aria-label="Move Duration" type="number" min={0} max={10000} value={step.movement_duration_ms} onChange={(event) => onChange({ movement_duration_ms: Number(event.target.value) })} /></FieldRow><FieldRow title="Natural Randomness" info="Adds imperfect line wobble and timing variation."><Input aria-label="Natural Randomness" type="range" min={0} max={100} value={step.natural_randomness} onChange={(event) => onChange({ natural_randomness: Number(event.target.value) })} /></FieldRow><FieldRow title="Overshoot Chance (%)" info="Chance to pass target slightly before correcting back."><Input aria-label="Natural Overshoot Chance" type="number" min={0} max={100} value={step.natural_overshoot_chance} onChange={(event) => onChange({ natural_overshoot_chance: Number(event.target.value) })} /></FieldRow><FieldRow title="Overshoot Max (px)" info="Maximum overshoot distance before correction."><Input aria-label="Natural Overshoot Px" type="number" min={0} max={500} value={step.natural_overshoot_px} onChange={(event) => onChange({ natural_overshoot_px: Number(event.target.value) })} /></FieldRow><FieldRow title="Overshoot Severity" info="Scales overshoot distance and correction drama."><Input aria-label="Natural Overshoot Severity" type="range" min={0} max={100} value={step.natural_overshoot_severity} onChange={(event) => onChange({ natural_overshoot_severity: Number(event.target.value) })} /></FieldRow><FieldRow title="Period Min (px)" info="Minimum distance between cosine peaks."><Input aria-label="Natural Period Min" type="number" min={6} max={200} value={step.natural_period_min_px} onChange={(event) => onChange({ natural_period_min_px: Number(event.target.value) })} /></FieldRow><FieldRow title="Period Max (px)" info="Maximum distance between cosine peaks."><Input aria-label="Natural Period Max" type="number" min={6} max={300} value={step.natural_period_max_px} onChange={(event) => onChange({ natural_period_max_px: Number(event.target.value) })} /></FieldRow><FieldRow title="Amplitude Min (px)" info="Minimum cosine peak amplitude from straight path."><Input aria-label="Natural Amplitude Min" type="number" min={0} max={60} value={step.natural_amplitude_min_px} onChange={(event) => onChange({ natural_amplitude_min_px: Number(event.target.value) })} /></FieldRow><FieldRow title="Amplitude Max (px)" info="Maximum cosine peak amplitude from straight path."><Input aria-label="Natural Amplitude Max" type="number" min={0} max={80} value={step.natural_amplitude_max_px} onChange={(event) => onChange({ natural_amplitude_max_px: Number(event.target.value) })} /></FieldRow><FieldRow title="Peak Reversal Chance (%)" info="Chance each segment flips side to become the opposite peak/trough."><Input aria-label="Natural Peak Reversal Chance" type="number" min={0} max={100} value={step.natural_peak_reversal_chance} onChange={(event) => onChange({ natural_peak_reversal_chance: Number(event.target.value) })} /></FieldRow>{step.natural_period_max_px < step.natural_period_min_px ? <p className="text-xs text-warning">Period Max should be greater than or equal to Period Min.</p> : null}{step.natural_amplitude_max_px < step.natural_amplitude_min_px ? <p className="text-xs text-warning">Amplitude Max should be greater than or equal to Amplitude Min.</p> : null}<CoordinatePreview x={step.x} y={step.y} offset={0} exactTarget /></> : <><FieldRow title="Position Randomness (px)" info="Random offset around target point."><Input type="number" value={step.random_offset} onChange={(event) => onChange({ random_offset: Number(event.target.value) })} /></FieldRow><CoordinatePreview x={step.x} y={step.y} offset={step.random_offset} /></>}</> : null}
