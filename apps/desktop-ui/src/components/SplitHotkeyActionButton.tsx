...
export function SplitHotkeyActionButton({
  ...
  return (
    <div className={`inline-flex overflow-hidden rounded-lg border shadow-raycast ${toneClass}`}>
      ...
      <button type="button" className={`border-l px-2 transition ${separatorClass} ${tone === "danger" ? "hover:bg-danger/24 hover:shadow-[inset_0_0_0_1px_hsl(var(--danger)/0.5)] active:bg-danger/28" : "hover:bg-success/24 hover:shadow-[inset_0_0_0_1px_hsl(var(--success)/0.5)] active:bg-success/28"}`} onClick={onHotkey}>
