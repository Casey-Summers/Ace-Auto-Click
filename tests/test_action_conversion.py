...
from ace_auto_click.api.models import ClickStepModel, DragStepModel, KeyTapStepModel, MoveStepModel
from ace_auto_click.automation.actions import ClickStep, DragStep, KeyTapStep, MoveStep
...
def test_api_drag_model_converts_to_runtime_step() -> None:
    step = _to_action_step(DragStepModel(id="drag-1", x=10, y=20, angle_degrees=90, distance_px=300))

    assert isinstance(step, DragStep)
    assert step.x == 10
    assert step.y == 20
    assert step.buttons == ["left", "right"]
    assert step.angle_degrees == 90
    assert step.distance_px == 300
