...
application, not a browser tab. Development mode starts Python, Vite, and
Cargo/Tauri tooling, so it is not the fastest startup path:

```powershell
python app.py desktop
```

For daily use, build and start the release desktop app. This avoids the Vite
dev server and is the standard path for near-instant startup:
